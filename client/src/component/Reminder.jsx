  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6">
      <div className="mb-4 flex items-center justify-between">
        ...
        <button
          onClick={fetchData}
          className="rounded-xl bg-gradient-to-r from-blue-600 to-blue-500 px-4 py-2 text-white shadow hover:from-blue-500 hover:to-blue-400 active:scale-[.98]"
        >
          重新整理
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-red-600">{err}</div>
      ) : reminders.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-300 p-8 text-center text-slate-500">
          目前沒有提醒事項
        </div>
            ...
            {reminders.map((_, i) => (
              <button
                ...
                className={`h-2 rounded-full transition-all ${
                  i === index ? "w-6 bg-gradient-to-r from-blue-500 to-sky-400" : "w-2 bg-slate-300"
                }`}
        ...
        >
          ...
          >
            <div className="flex items-center justify-between bg-gradient-to-r from-orange-500 to-orange-400 px-5 py-4 text-white">
              <div className="text-lg font-extrabold">{current.title || "（未命名）"}</div>
              <button
            ...
            <div className="flex justify-end gap-2 border-t border-slate-100 px-5 py-3">
              ...
              <button
                onClick={() => setIsOpen(false)}
                className="rounded-xl bg-gradient-to-r from-blue-600 to-blue-500 px-4 py-2 font-semibold text-white hover:from-blue-500 hover:to-blue-400"
              >
                確定
              </button>
            </div>
