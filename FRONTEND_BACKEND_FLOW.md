...
| `paymentApi.js` | `listPayments`、`updatePayment` | `/classes/:scheduleId/payments` |
...
| `usePayments.js` | 繳費列表、更新繳費 | `paymentApi.js` |
...
| `Payment.jsx` | 繳費列表容器 | `usePayments(scheduleId)` | 更新繳費 |
| `PaymentCard.jsx` | 單一學生繳費卡 | `Payment.jsx` props | `onEdit(payment)` |
| `AddPayment.jsx` | 更新繳費 modal | 元件內 amount/status state | `onSave(payment)` 給 `Payment.jsx` |
...
| `payment.js` | `GET/PATCH /classes/:scheduleId/payments`，也保留舊路徑 | `paymentController` |
...
| `paymentController.js` | 繳費列表、更新繳費 | `paymentModel`、`withTransaction` |
...
| `classModel.js` | `class`、`class_members`、`students`、`payments` | `getClassesByUserId`、`findClassByScheduleForUser`、`upsertStudent`、`createClassMember` |
| `reminderModel.js` | `reminders`、`schedule` | `findOwnedSchedule`、`createReminder`、`listByUserId`、`deleteByIdForUser` |
| `paymentModel.js` | `payments`、`class_members`、`students` | `listBySchedule`、`findClassMemberForPayment`、`createPayment`、`updatePayment` |
...
-> withTransaction
   ...
   -> assignSeatToStudent
      ...
      -> createClassMember
   -> ensurePaymentForClassMember
      -> findPaymentByClassMemberId
      -> createInitialPayment 或 updatePaymentStudentName
-> success
-> refresh seats
```
...
-> paymentApi.listPayments(scheduleId)
-> GET /classes/:scheduleId/payments
-> paymentController.listPayments
-> scheduleIdParam 驗證
-> paymentModel.listBySchedule({ scheduleId, userId })
-> SQL JOIN class + schedule + class_members + students
-> LEFT JOIN payments
-> 回傳每個學生的繳費狀態
-> PaymentCard 顯示
```

`LEFT JOIN payments` 的意思是：即使某個學生還沒有 payment row，也會出現在畫面上，只是金額和狀態會使用預設值。
...
PaymentCard.jsx
-> 使用者點 edit
-> Payment.jsx setStudent / setShowUp(true)
-> AddPayment.jsx
-> 使用者輸入 amount/status
-> Payment.jsx handleSavePayment
-> usePayments.savePayment
-> paymentApi.updatePayment
-> PATCH /classes/:scheduleId/payments
-> paymentController.updatePayment
-> paymentUpdateRequest 驗證
-> withTransaction
   -> paymentModel.findClassMemberForPayment
   -> paymentModel.findByClassMemberId
   -> 如果沒有 payment：paymentModel.createPayment
   -> 如果已有 payment：paymentModel.updatePayment
-> success
-> refresh payments
```
...
| 繳費列表 | GET | `/classes/:scheduleId/payments` | `routes/payment.js` | `paymentController.listPayments` | `paymentModel.listBySchedule` |
| 更新繳費 | PATCH | `/classes/:scheduleId/payments` | `routes/payment.js` | `paymentController.updatePayment` | `paymentModel.*` |

後端目前也保留一些舊 endpoint，例如 `/course`、`/showcourse`、`/class`、`/seat/:scheduleId`、`/payment/:scheduleId`。新程式優先使用 REST 風格的 `/courses`、`/classes/...`。
...
| 更新座位 | `students` | `class_members`、`payments` |
| 提醒 | `reminders` | `schedule` |
| 繳費 | `payments` | `class_members`、`students`、`class`、`schedule` |
