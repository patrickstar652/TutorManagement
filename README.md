...
- 繳費管理：依收費期別設定堂數與單價，固定每位學生應繳金額，只更新已繳／未繳狀態並即時統計款項。
...
- `GET /classes/:scheduleId/payments?periodId=<id>`：取得最新或指定歷史期別的繳費總覽
- `POST /classes/:scheduleId/billing-periods`：建立收費期別並快照目前班級名單
- `PATCH /classes/:scheduleId/billing-periods/:periodId`：修改尚未有人繳費的期別
- `PATCH /classes/:scheduleId/payments/:paymentId`：只更新單一學生的 `已繳`／`未繳`狀態

收費期別規則：

- 每人應繳金額為「實際堂數 × 每堂單價」，同一期別全班金額一致。
- 建立期別時會快照當下班級名單；之後的座位異動不會改動歷史期別。
- 同一班級的收費日期不可重疊，且班級沒有學生時不可建立期別。
- 只要仍有任一學生標記為已繳，該期別的日期、堂數與單價就不可修改。
- 未指定 `periodId` 時預設顯示最新期別；總覽包含總應收、已收、未收與人數統計。
...
- 現有程式以手動驗證為主，修改後建議至少執行前端 `lint` 與 `build`，並手動測試登入、課程、座位、提醒，以及建立／切換收費期別、更新繳費狀態與統計流程。

## 後續可補強方向

- 補齊自動化測試
- 統一前端 API base URL 設定
- 補齊 migration 的部署與回復流程文件
- 強化登入與密碼安全性
- 補齊部署與正式環境設定文件
